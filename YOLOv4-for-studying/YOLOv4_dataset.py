...
class Dataset(object):
    ...
    def preprocess_true_bboxes(self, bboxes):       #bbox: [xy_min, xy_max]
        ...
        label = [np.zeros((self.output_gcell_sizes_h[i], self.output_gcell_sizes_w[i], self.num_anchors_per_gcell_small if (i==0 and USE_5_ANCHORS_SMALL_SCALE) else self.num_anchors_per_gcell, PRED_NUM_PARAMETERS + self.num_classes), dtype=np.float32)
        ...
        for bbox in bboxes:
            ...
            for i in range(self.num_output_levels):
                ...
                left, top = np.maximum(np.floor(bbox_xywh_scaled[i, :2]-1).astype(np.int32), 0)
                right, bottom = np.minimum(np.ceil(bbox_xywh_scaled[i, :2]+2).astype(np.int32), [self.output_gcell_sizes_w[i], self.output_gcell_sizes_h[i]])
                if USE_5_ANCHORS_SMALL_SCALE:
                ...
                if np.any(iou_mask):
                    ...
                    label[i][row, column, iou_mask, 5:(5+self.num_classes)] = smooth_onehot #class probabilities
                    if PRED_NUM_PARAMETERS == 6:
                        label[i][top:bottom,left:right, iou_mask, -1] = 1.0
            ...
            if not has_positive_flag:
                ...
                left, top = np.maximum(np.floor(bbox_xywh_scaled[best_scale_idx, :2]-1).astype(np.int32), 0)
                right, bottom = np.minimum(np.ceil(bbox_xywh_scaled[best_scale_idx, :2]+2).astype(np.int32), [self.output_gcell_sizes_w[best_scale_idx], self.output_gcell_sizes_h[best_scale_idx]])
                ...
                label[best_scale_idx][row, column, best_anchor_idx, 5:(5+self.num_classes)] = smooth_onehot
                label[best_scale_idx][top:bottom,left:right, best_anchor_idx, -1] = 1.0
    ...
    def __next__(self):
        with tf.device('/cpu:0'):
            ...
            batch_label_sbboxes = np.zeros((self.batch_size, self.output_gcell_sizes_h[0], self.output_gcell_sizes_w[0],
                                            self.num_anchors_per_gcell_small if USE_5_ANCHORS_SMALL_SCALE else self.num_anchors_per_gcell, PRED_NUM_PARAMETERS + self.num_classes), dtype=np.float32)
            batch_label_mbboxes = np.zeros((self.batch_size, self.output_gcell_sizes_h[1], self.output_gcell_sizes_w[1],
                                            self.num_anchors_per_gcell, PRED_NUM_PARAMETERS + self.num_classes), dtype=np.float32)
