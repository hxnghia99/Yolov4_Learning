...
def main():
    ...
    writer = tf.summary.create_file_writer(TRAIN_LOGDIR+"/training")
    ...
    yolo = Create_Yolo(input_size=YOLO_INPUT_SIZE, training=True, CLASSES=LG_CLASSES)
    if TRAIN_FROM_CHECKPOINT:
        try:
            yolo.load_weights(f"./YOLOv3-for-studying/TensorFlow-2.x-YOLOv3/checkpoints/{TRAIN_MODEL_NAME}")
        except ValueError:
    ...
    def train_step(image_data, target):
        with tf.GradientTape() as tape:
            ...
            for i in range(grid):
                ...
                loss_items = compute_loss(pred, conv, *target[i], i, CLASSES=LG_CLASSES)
    ...
    validate_writer = tf.summary.create_file_writer(TRAIN_LOGDIR+"/validating")
    ...
    for epoch in range(TRAIN_EPOCHS):
        ...
        print("giou_val_loss:{:7.2f}, conf_val_loss:{:7.2f}, prob_val_loss:{:7.2f}, total_val_loss:{:7.2f} \n".
              format(giou_val/steps_per_epoch, conf_val/steps_per_epoch, prob_val/steps_per_epoch, total_val/steps_per_epoch))
